...
class TrialParamHandler(object):
    ...
    def __init__(self, sph):
        ...
        self.laser_stimulation= int(np.random.choice([1, 0], p=[tph.laser_probability, 1-tph.laser_probability])) # BNC Output 255

        # [iti, quiescence, go_cue]

        if self.laser_stimulation==1:
            #self.laser_stim_where = np.random.choice(['iti', 'quiescence', 'go_cue'], p=[0.33, 0.33, 0.34])
            ...
            self.laser_stim_where = self.out_laser_off
