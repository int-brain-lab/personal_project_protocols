...
# this initiates just a first trial
    def __init__(self, sph):
        ...
        self.contrast_set = sph.CONTRAST_SET # contrast
        self.contrast_laser_set = sph.CONTRAST_LASER_SET # defines which contrasts are with laser stimulation
        ...
        self.laser_probability=sph.LASER_PROBABILITY #0.6 # set laser probability sph.LASER_PROBABILITY
        self.prestimulus_time_laser=sph.PRESTIMULUS_TIME_LASER
        self.laser_stimulation_where=sph.LASER_STIMULATION_WHERE
        self.probability_laser_stimulation_where=sph.PROBABILITY_LASER_STIMULATION_WHERE
        self.p_laser_where=sph.PROBABILITY_LASER_STIMULATION_WHERE
        ...
        #self.contrast - information about contrastsvalue;



        if self.contrast in self.contrast_laser_set: # [1.0, 0.125, 0.0]

            self.laser_stimulation= int(np.random.choice([1, 0], p=[self.laser_probability, 1-self.laser_probability])) # BNC Output 255

        else:

            self.laser_stimulation=0

      

        #if self.contrast ==[]

        #self.laser_stimulation= int(np.random.choice([1, 0], p=[laser_probability, 1-laser_probability])) # BNC Output 255

        #else

        #self.laser_stimulation=self.out_laser_off



        # [iti, quiescence, go_cue]



        if self.laser_stimulation==1:

            self.laser_stim_where = np.random.choice(self.laser_stimulation_where, p=self.p_laser_where)#p=[0,0,1,0])

            #self.laser_stim_where = np.random.choice(tph.laser_stimulation_where, p=probability_laser_stimulation_where) # this did not work for me

            self.laser_out_iti = self.out_laser_on if self.laser_stim_where == 'iti' else self.out_laser_off

            self.laser_out_quiescence = self.out_laser_on if self.laser_stim_where == 'quiescence' else self.out_laser_off

            self.laser_out_go_cue = self.out_laser_on if self.laser_stim_where == 'go_cue' else self.out_laser_off

            self.laser_out_stimulus = self.out_laser_on if self.laser_stim_where == 'stimulus_on' else self.out_laser_off

            self.laser_out = self.out_laser_on

        else:

            self.laser_stim_where = 'None'

            self.laser_out_iti = self.out_laser_off

            self.laser_out_quiescence = self.out_laser_off

            self.laser_out_go_cue = self.out_laser_off

            self.laser_out_stimulus=self.out_laser_off
...
LASER STIM WHERE      {self.laser_stim_where}
LASER OUT             {self.laser_out}
...
##########################################"""
    ...
    def next_trial(self):
        ...
        if self.laser_stimulation==1:
            self.laser_stim_where = np.random.choice(self.laser_stimulation_where, p=self.p_laser_where)#p=[0, 0, 1, 0])
            #self.laser_stim_where = np.random.choice(tph.laser_stimulation_where, probability_laser_stimulation_where)
            self.laser_out_iti = self.out_laser_on if self.laser_stim_where == 'iti' else self.out_laser_off
            self.laser_out_quiescence = self.out_laser_on if self.laser_stim_where == 'quiescence' else self.out_laser_off
            self.laser_out_go_cue = self.out_laser_on if self.laser_stim_where == 'go_cue' else self.out_laser_off
            self.laser_out_stimulus = self.out_laser_on if self.laser_stim_where == 'stimulus_on' else self.out_laser_off
